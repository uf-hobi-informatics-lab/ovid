...
###################################################################################################################################
    ...
    def copy_data(cls,input_file_name,output_file_name,data_folder_path,output_data_folder_path ):
        ...
        command = ["cp", os.path.join(data_folder_path, input_file_name), os.path.join(output_data_folder_path,output_file_name)]
                # Execute the command
        subprocess.run(" ".join(command), shell=True)
...
###################################################################################################################################
    ...
    def copy_folder(cls,folder_to_copy_from, folder_to_copy_to):

        
        if os.path.exists(folder_to_copy_to):

                userInput = input('Folder already exists. Would you like to overwrite it? (Y/n): ')
                ...
                shutil.rmtree(folder_to_copy_to)

        # print("Copying folder...")
        shutil.copytree(folder_to_copy_from, folder_to_copy_to)
        # print("Folder copy completed.")
        for root, dirs, files in os.walk(folder_to_copy_to):
                for d in dirs:
                        os.chmod(os.path.join(root, d), 0o777)
                for f in files:
                        os.chmod(os.path.join(root, f), 0o777)
        os.chmod(folder_to_copy_to, 0o777)
...
###################################################################################################################################
    ...
    def datavant_transfer_tokens_to_pcornet(cls,input_file_path, file_name, output_file_path,folder,partner):
        ...
        datavant_key = DATAVANT_KEYS.get(partner.upper())
        ...
        for file_name in file_names_only:
            ...
            with open(DV_CREDENTIALS_PATH, "rb") as cred_file:
                process = subprocess.Popen(
                    [ 
                        DV_TRANSFORMATION_SCRIPT_PATH,
                        "transform-tokens",
                        "--to", "pcornet",
                        "--from", datavant_key,
                        "-s", "oneflorida_crn",
...
###################################################################################################################################
    ...
    def get_mothly_counts(cls,partner,file_name,table_name, file_path,folder_name):
        ...
        spark = SparkSession.builder.master("spark://master:7077").appName("MONTHLY COUNT").getOrCreate()
    ...
    def get_distinct_values(cls,partner,file_name,table_name, file_path,folder_name):
        ...
        if file_name.lower() not in bypass_files_list:
            ...
            for col_name in df.columns:
                ...
                if 'DATE' not in col_name and not col_name.endswith("ID") :
...
###################################################################################################################################
    ...
    @classmethod
    def db_upload_cdm_complementary_tables(cls,db, db_server,folder_name,db_type):
        ...
        if db_type.upper() == 'SF':
            cls.db_upload_cdm_complementary_tables_snowflake(db, db_server,folder_name)
...
###################################################################################################################################
    ...
    def db_upload_cdm_complementary_tables_snowflake(cls, db, db_server, folder_name):
...
###################################################################################################################################
    ...
    def db_upload_snowflake(cls, db, db_server, schema, file_name, table_name, file_path, folder_name):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(f"CREATE SCHEMA IF NOT EXISTS {db}.OVID_{folder_name}")
            cur.close()
            conn.close()
...
###################################################################################################################################
    ...
    def check_scheduler_db_exists(cls):
        ...
        # Load PKCS8 (.p8) private key for authentication
...
###################################################################################################################################
    ...
    def get_spark_session(cls, appName):
        ...
        spark = SparkSession.builder.master("spark://master:7077").appName(appName).config("spark.sql.parquet.datetimeRebaseModeInRead", "LEGACY").getOrCreate()
...
###################################################################################################################################
    ...
    def print_with_style(cls, text, color):
        ...
        if color == 'italic white':
            bold_code = '\033[1m'
            color_code = '\033[97m'  # bright white
            reset_code = '\033[0m'   # Reset to default style
